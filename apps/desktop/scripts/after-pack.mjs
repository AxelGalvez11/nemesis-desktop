...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Nemesis'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Nemesis.exe keeps the stock Electron icon`)
  }
}
