...
export const NemesisAccountGate = () => {
    ...
    if (account.status === 'signed-out') {
        return (_jsx("div", { className: "fixed inset-0 z-[1300] flex items-center justify-center bg-background/95 backdrop-blur-md p-4", children: _jsxs("div", { className: "w-full max-w-sm rounded-2xl border border-border bg-card p-8 shadow-lg", children: [_jsxs("div", { className: "flex flex-col items-center gap-3 pb-5 text-center", children: [_jsx(BrandMark, { className: "size-12" }), _jsxs("div", { children: [_jsx("h2", { className: "text-lg font-semibold tracking-tight", children: "Sign in to Nemesis" }), _jsx("p", { className: "pt-1 text-xs text-muted-foreground", children: "Use your Nemesis account. Your plan stays in sync across the desktop and account portal." })] })] }), _jsxs("div", { className: "flex flex-col gap-2.5", children: [['google', 'apple'].map(provider => (_jsx(Button, { onClick: () => {
                                    setOauthStarted(provider);
                                    void window.hermesDesktop?.openExternal?.(desktopOAuthStartUrl(provider));
                                }, variant: "secondary", children: oauthStarted === provider
                                    ? 'Browser opened — finish there, or click to reopen'
                                    : `Continue with ${provider === 'google' ? 'Google' : 'Apple'}` }, provider))), oauthStarted && (_jsx("p", { "aria-live": "polite", className: "text-center text-xs text-muted-foreground", children: oauthSlow
