...
export type UpdaterStatus = 'downloaded' | 'error' | 'unavailable' | 'working'

export interface RelaunchCardState {
  kind: 'manual' | 'relaunch'
  version: null | string
}
...
export function relaunchCardState(
  updater: UpdaterStatus,
  latestTag: null | string,
  currentVersion: null | string
): null | RelaunchCardState {
...
export const NemesisRelaunchCard: FC = () => {
  ...
  return (
    <button
      aria-label={state.kind === 'relaunch' ? 'Relaunch to update' : 'Update available'}
      className="group/update flex w-full items-center gap-2 rounded-lg border border-(--ui-stroke-tertiary) bg-(--ui-bg-card) px-2 py-1.5 text-left transition-colors duration-100 ease hover:border-(--theme-primary)/40 hover:bg-(--ui-control-hover-background) active:scale-[0.99] motion-reduce:active:scale-100"
      onClick={activate}
      type="button"
    >
      <span className="grid size-6 shrink-0 place-items-center rounded-md bg-(--theme-primary)/12 text-(--theme-primary)">
        <Codicon name={state.kind === 'relaunch' ? 'arrow-up' : 'cloud-download'} size="0.8rem" />
      </span>
      <span className="min-w-0 flex-1">
        <span className="block truncate text-xs font-medium text-foreground">
          {state.kind === 'relaunch' ? 'Relaunch to update' : 'Update available'}
        </span>
        <span className="block truncate text-[0.65rem] text-(--ui-text-tertiary)">
          {state.version ? `v${state.version}` : 'A new version is ready'}
        </span>
      </span>
      <Codicon
        className="shrink-0 text-(--ui-text-tertiary) transition-transform duration-100 group-hover/update:translate-x-0.5 group-hover/update:text-foreground"
        name="arrow-right"
        size="0.8rem"
      />
    </button>
