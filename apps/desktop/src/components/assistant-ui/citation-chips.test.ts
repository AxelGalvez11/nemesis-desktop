...
describe('linkifyCitations', () => {
  it('turns a parenthesized PMID into a labeled PubMed link, dropping the parens', () => {
    expect(linkifyCitations('reduced liver fat (PMID: 38858523).')).toBe(
      'reduced liver fat [PubMed](https://pubmed.ncbi.nlm.nih.gov/38858523/).'
    )
  })

  it('splits a multi-citation parenthetical into one chip per source', () => {
    expect(linkifyCitations('(PMID: 39318607, PMID: 40291085)')).toBe(
      '[PubMed](https://pubmed.ncbi.nlm.nih.gov/39318607/) [PubMed](https://pubmed.ncbi.nlm.nih.gov/40291085/)'
    )
  })

  it('links bare PMIDs and NCT trial ids', () => {
    expect(linkifyCitations('see PMID 37385280 and NCT04881760')).toBe(
      'see [PubMed](https://pubmed.ncbi.nlm.nih.gov/37385280/) and [Trial](https://clinicaltrials.gov/study/NCT04881760)'
    )
  })
