...
function linkifySegment(segment: string): string {
  return segment
    ...
    .replace(PMID_RE, (_match, pmid: string) => `[PubMed](${pubmedUrl(pmid)})`)
    .replace(NCT_RE, (_match, nct: string) => `[Trial](${trialUrl(nct)})`)
}
