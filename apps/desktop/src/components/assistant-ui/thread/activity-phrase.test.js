...
import { ACTIVITY_FALLBACK_PHRASE, currentActivityEvent, phraseForActivity } from './activity-phrase';
const tool = (toolName, args) => ({ args, toolName, type: 'tool-call' });
...
describe('phraseForActivity', () => {
    ...
    it('maps web search to source + quoted query', () => {
        expect(phraseForActivity(tool('web_search', { query: 'tesamorelin dosing' }))).toBe("Searching the web for 'tesamorelin dosing'…");
    });
    it('maps pubmed-flavored search tools to PubMed', () => {
        expect(phraseForActivity(tool('pubmed_search', { query: 'tesamorelin' }))).toBe("Searching PubMed for 'tesamorelin'…");
    });
    it('maps trials-flavored search tools to ClinicalTrials.gov', () => {
        expect(phraseForActivity(tool('search_trials', { query: 'GLP-1 agonists' }))).toBe("Searching ClinicalTrials.gov for 'GLP-1 agonists'…");
    });
    it('maps openfda-flavored search tools to openFDA', () => {
        expect(phraseForActivity(tool('openfda_search', { search_term: 'metformin recalls' }))).toBe("Searching openFDA for 'metformin recalls'…");
    });
    ...
    it('maps file writes and edits to the file basename', () => {
        expect(phraseForActivity(tool('write_file', { path: '/Users/ax/Library/Renal dosing.md' }))).toBe('Writing Renal dosing.md…');
        expect(phraseForActivity(tool('edit_file', { file: 'notes\\pharm\\Loop diuretics.md' }))).toBe('Writing Loop diuretics.md…');
    ...
    it('names the purpose of recognizable terminal commands without echoing them', () => {
        expect(phraseForActivity(tool('bash', { command: 'curl -s "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=tesamorelin"' }))).toBe('Searching PubMed…');
        expect(phraseForActivity(tool('bash', { command: 'python3 build_deck.py --out slides.pptx' }))).toBe('Building your slides…');
        expect(phraseForActivity(tool('bash', { command: "osascript -e 'tell application \"Mail\" to get subject of messages 1 thru 25 of inbox'" }))).toBe('Reading your inbox…');
    ...
    it('maps the nemesis-study-decks skill to the flashcard phrase', () => {
        expect(phraseForActivity(tool('skill_view', { name: 'nemesis-study-decks' }))).toBe('Building your flashcard deck…');
    });
    ...
    it('maps the school-portal skill (plugin-qualified too) to the portal phrase', () => {
        ...
        expect(phraseForActivity(tool('skill_view', { name: 'nemesis:school-portal' }))).toBe('Checking your school portal…');
    });
