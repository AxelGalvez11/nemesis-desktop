...
export interface ActivityReasoningEvent {
  text: string
  type: 'reasoning'
}

export type ActivityEvent = ActivityReasoningEvent | ActivityToolEvent
...
function searchSource(toolName: string): string {
...
function searchPhrase(source: string, args: Record<string, unknown>): string {
...
function fileEditPhrase(args: Record<string, unknown>): string {
...
function skillPhrase(args: Record<string, unknown>): string {
...
function reasoningPhrase(text: string): string {
 ...
 */
export function phraseForActivity(event: ActivityEvent | null | undefined): string {
  if (!event) {
    return ACTIVITY_FALLBACK_PHRASE
  }
 ...
 */
export function currentActivityEvent(parts: ReadonlyArray<unknown>): ActivityEvent | null {
