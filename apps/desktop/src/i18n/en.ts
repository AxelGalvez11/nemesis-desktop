...
export const en: Translations = {
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed:
        'The desktop backend rejected that request (405 Method Not Allowed). Try restarting Nemesis Desktop.',
  ...
  settings: {
    ...
    appearance: {
      ...
      pet: {
        title: 'Pet',
        intro:
          'Adopt an animated petdex mascot that floats over the app and reacts to what Nemesis is doing — running while tools execute, celebrating on success, sulking on errors.',
        restartHint:
          'Pets need a quick restart — the running app started before this feature was added. Quit and reopen Nemesis, then come back here.',
        on: 'On',
        off: 'Off',
        scaleTitle: 'Size',
    ...
    about: {
      ...
      automaticUpdatesDesc:
        'Nemesis checks for updates automatically in the background and lets you know when one is ready.',
    ...
    gateway: {
      ...
      intro:
        'Nemesis Desktop starts its own local gateway by default. Use a remote gateway when you want this app to control an already-running Nemesis backend on another machine or behind a trusted proxy. Pick a profile below to give it its own remote host.',
      ...
      remoteDesc:
        'Connect this desktop shell to a remote Nemesis backend. Hosted gateways use OAuth or a username and password; self-hosted ones may use a session token.',
    ...
    providers: {
      ...
      intro:
        'Sign in with a subscription — no API key to copy. Nemesis runs the browser sign-in for you, right here in the app.',
      connected: 'Connected',
      collapse: 'Collapse',
  ...
  cron: {
    ...
    newCron: 'New cron',
    emptyDescNew:
      'Schedule a prompt to run on a cron expression. Nemesis will run it and deliver results to the destination you pick.',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'Update the Nemesis backend to create projects — your backend is older than this desktop app (Settings → Updates → Backend).',
  ...
  updates: {
    ...
    guiSkewBody:
      'The backend was updated, but this desktop app package wasn’t changed. Update or reinstall the Nemesis desktop app (your AppImage / .deb / .rpm) to match.',
    copy: 'Copy',
    copied: 'Copied',
    done: 'Done',
    applyingBody:
      'The Nemesis updater takes over in its own window and reopens Nemesis automatically when it’s done. Please don’t reopen Nemesis yourself while it’s updating.',
    applyingBodyBackend:
      'The remote backend is applying the update and will restart. Nemesis reconnects automatically when it’s back.',
  ...
  install: {
    ...
    failedDesc:
      'One of the setup steps didn’t finish. Close any other Nemesis windows, then retry — the details below show exactly what happened.',
    activeDesc:
      'First launch only — Nemesis is downloading its study tools and setting itself up on this computer. This takes a few minutes and won’t happen again.',
  ...
  onboarding: {
    ...
    autoBrowser: provider =>
      `We opened ${provider} in your browser. Authorize Nemesis there and you'll be connected automatically — nothing to copy or paste.`,
  ...
  preview: {
    ...
    web: {
      ...
      stillWorking:
        'Nemesis is still working, but no restart result has arrived yet. The server command may be running in the foreground.',
  ...
  assistant: {
    ...
    approval: {
      ...
      alwaysDescription: pattern =>
        `This adds the “${pattern}” pattern to your permanent allowlist (~/.hermes/config.yaml). Nemesis won’t ask again for commands like this — in this session or any future one.`,
