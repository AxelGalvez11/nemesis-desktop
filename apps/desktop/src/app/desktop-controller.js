...
import { CRON_ROUTE, NEW_CHAT_ROUTE, routeSessionId, sessionRoute, SETTINGS_ROUTE, WELCOME_ROUTE } from './routes';
...
export function DesktopController() {
    ...
    const overlays = (_jsxs(_Fragment, { children: [_jsx(RemoteDisplayBanner, {}), !isSecondaryWindow() && _jsx(DesktopInstallOverlay, {}), !isSecondaryWindow() && _jsx(NemesisAccountGate, {}), !isSecondaryWindow() && _jsx(NemesisConsentGate, {}), !isSecondaryWindow() && !NEMESIS_STUDENT_BUILD && (_jsx(DesktopOnboardingOverlay, { enabled: gatewayState === 'open', onCompleted: () => {
                    ...
                    void queryClient.invalidateQueries({ queryKey: ['model-options'] });
                        ...
                        void refreshCurrentModel();
                        void queryClient.invalidateQueries({ queryKey: ['model-options'] });
                    } }) })), commandCenterOpen && !NEMESIS_STUDENT_BUILD && (_jsx(Suspense, { fallback: null, children: _jsx(CommandCenterView, { initialSection: commandCenterInitialSection, onClose: closeOverlayToPreviousRoute, onDeleteSession: removeSession, onNavigateRoute: path => navigate(path), onOpenSession: sessionId => navigate(sessionRoute(sessionId)) }) })), agentsOpen && !NEMESIS_STUDENT_BUILD && (_jsx(Suspense, { fallback: null, children: _jsx(AgentsView, { onClose: closeOverlayToPreviousRoute }) })), cronOpen && !NEMESIS_STUDENT_BUILD && (_jsx(Suspense, { fallback: null, children: _jsx(CronView, { onClose: closeOverlayToPreviousRoute, onOpenSession: sessionId => navigate(sessionRoute(sessionId)) }) })), profilesOpen && !NEMESIS_STUDENT_BUILD && (_jsx(Suspense, { fallback: null, children: _jsx(ProfilesView, { onClose: closeOverlayToPreviousRoute }) })), starmapOpen && !NEMESIS_STUDENT_BUILD && (_jsx(Suspense, { fallback: null, children: _jsx(StarmapView, { onClose: closeOverlayToPreviousRoute }) }))] }));
    ...
    const coldStartView = studentColdStart ? (sessionsLoading ? null : !onboardingComplete() && sessionsTotal === 0 ? (_jsx(Navigate, { replace: true, to: WELCOME_ROUTE })) : (chatView)) : (chatView);
    ...
    const reviewPane = (_jsx(Pane, { defaultOpen: true, 
        ...
        disabled: NEMESIS_STUDENT_BUILD || !chatOpen || !currentCwd.trim() || (!narrowViewport && !reviewOpen), forceCollapsed: narrowViewport, hoverReveal: true, id: REVIEW_PANE_ID, maxWidth: FILE_BROWSER_MAX_WIDTH, minWidth: FILE_BROWSER_MIN_WIDTH, 
    ...
    const terminalPane = (_jsx(Pane, { bottomRow: terminalAsRow, defaultOpen: true, disabled: NEMESIS_STUDENT_BUILD || !terminalSidebarOpen, divider: true, height: "38vh", id: "terminal-sidebar", maxHeight: "80vh", maxWidth: "80vw", minHeight: "8rem", minWidth: "22vw", resizable: true, side: railSide, width: "42vw", children: _jsx("div", { className: cn('relative flex h-full min-h-0 min-w-0 flex-col overflow-hidden bg-(--ui-editor-surface-background)', terminalAsRow ? 'border-l border-(--ui-stroke-secondary) pt-0' : 'pt-(--titlebar-height)'), children: _jsx(TerminalPaneChrome, {}) }) }, "terminal-sidebar"));
    const noteChatPane = (_jsx(Pane, { defaultOpen: false, disabled: !noteChatContext, forceCollapsed: narrowViewport, hoverReveal: true, id: NOTE_CHAT_PANE_ID, maxWidth: 520, minWidth: 300, overlayWidth: 320, resizable: true, side: railSide, width: "360px", children: noteChatContext ? _jsx(NoteChatPanel, { context: noteChatContext }) : null }, "note-chat"));
