...
import { mirrorTestAttempts, readDiskStudyState, readDiskTestAttempts, STUDY_STATE_FILE, TEST_ATTEMPTS_FILE } from './disk-state';
const files = new Map();
function stubBridge() {
    vi.stubGlobal('window', {
        hermesDesktop: {
            ...
            readFileText: async (path) => ({ text: files.get(path) ?? '' }),
...
const EMPTY_STATE = { decks: [], reviews: [], schedule: {}, sections: [], version: 1 };
beforeEach(() => {
    files.clear();
    vi.useFakeTimers();
    stubBridge();
...
describe('disk mirror', () => {
    it('debounces writes and lands the LAST store as a versioned envelope', async () => {
        const first = { 'a.json': { attempts: [{ date: '2026-07-16', score: 1, total: 4 }] } };
        const second = {
        ...
        const written = JSON.parse(files.get(TEST_ATTEMPTS_FILE) ?? 'null');
