...
import { applyPhoneReviews, buildPhoneDecksPayload, type PhoneReviewRow } from './phone-sync'

const NOW = new Date('2026-07-17T12:00:00Z')

function fixtureState(): StudyState {
  return {
    version: 1,
    decks: [
      {
...
describe('applyPhoneReviews', () => {
  const row = (over: Partial<PhoneReviewRow>): PhoneReviewRow => ({
  ...
  it('applies cloze-slot keys and skips keys for cards that no longer exist', () => {
    const result = applyPhoneReviews(
      fixtureState(),
      [row({ schedule_key: 'card-b#c2' }), row({ id: '00000000-0000-0000-0000-000000000002', schedule_key: 'card-deleted' })],
      NOW
    )
  ...
  it('skips unknown grades and clamps future timestamps to now', () => {
    const result = applyPhoneReviews(
      fixtureState(),
      [row({ grade: 'perfect' }), row({ id: '00000000-0000-0000-0000-000000000003', reviewed_at: '2030-01-01T00:00:00Z' })],
      NOW
    )
