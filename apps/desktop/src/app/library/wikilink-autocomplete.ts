...
export interface WikilinkTarget {
  title: string
  /** '' for root notes — shown as a `detail` hint so same-titled notes in different
   *  folders are distinguishable in the popup. */
  folder: string
}
...
export function wikilinkApplyText(title: string, textAfterCursor: string): string {
  const alreadyClosed = textAfterCursor.startsWith(']]') || textAfterCursor.startsWith('|')

  return alreadyClosed ? title : `${title}]]`
}

function applyWikilink(view: EditorView, completion: Completion, from: number, to: number): void {
...
export function wikilinkOptions(targets: readonly WikilinkTarget[]): Completion[] {
  return targets
    .slice()
...
export function wikilinkCompletionSource(
  getTargets: () => readonly WikilinkTarget[]
