...
export function keysForNote(note) {
 ...
 *  every keystroke. */
export function buildResolvableTitleSet(notes) {
    const set = new Set();
...
export function isWikilinkResolved(target, resolvable) {
...
export function findLinkedNote(target, notes) {
...
export function rewriteWikilinks(content, oldTitle, newTitle) {
    ...
    return content.replace(WIKILINK_FULL_RE, (full, rawTarget, heading = '', alias = '') => {
