...
export function PdfViewer({ path }) {
    ...
    if (error) {
        return (_jsx("div", { className: "grid h-full place-items-center rounded-2xl border border-(--ui-stroke-tertiary) bg-(--ui-bg-card) p-6 text-center text-sm text-muted-foreground", children: error }));
    }
    return (_jsxs("div", { className: "flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-(--ui-stroke-tertiary) bg-(--ui-bg-secondary,var(--color-muted)) shadow-[inset_0_1px_0_var(--ui-stroke-quaternary)]", children: [_jsxs("div", { className: "flex shrink-0 items-center justify-between gap-2 border-b border-(--ui-stroke-tertiary) bg-(--ui-bg-elevated) px-3 py-2", children: [_jsx("span", { className: "text-[0.6875rem] font-medium tabular-nums text-muted-foreground", children: doc ? `Page ${currentPage} / ${doc.numPages}` : 'Opening…' }), _jsxs("div", { className: "flex items-center gap-1", children: [_jsx(Button, { "aria-label": "Zoom out", onClick: () => step(-1), size: "icon-xs", variant: "ghost", children: "\u2212" }), _jsxs("span", { className: "w-12 text-center text-[0.6875rem] font-medium tabular-nums text-muted-foreground", children: [Math.round(scale * 100), "%"] }), _jsx(Button, { "aria-label": "Zoom in", onClick: () => step(1), size: "icon-xs", variant: "ghost", children: "+" }), _jsx(Button, { onClick: () => setZoom('fit'), size: "xs", variant: zoom === 'fit' ? 'secondary' : 'ghost', children: "Fit" })] })] }), _jsxs("div", { className: "flex min-h-0 flex-1", children: [doc && baseSize && doc.numPages > 1 && (_jsx("div", { className: "w-[6.5rem] shrink-0 overflow-y-auto border-r border-(--ui-stroke-tertiary) bg-(--ui-sidebar-surface-background) px-2 py-2", ref: railRef, children: Array.from({ length: doc.numPages }, (_, i) => (_jsx(PdfThumb, { active: currentPage === i + 1, baseSize: baseSize, doc: doc, onSelect: jumpToPage, pageNumber: i + 1, root: railRef.current }, i + 1))) })), _jsx("div", { className: "min-h-0 flex-1 overflow-y-auto px-4 py-2", onScroll: onScroll, ref: containerRef, children: doc && baseSize ? (Array.from({ length: doc.numPages }, (_, i) => (_jsx(PdfPage, { baseSize: baseSize, doc: doc, onHost: registerPageHost, pageNumber: i + 1, root: containerRef.current, scale: scale }, `${i + 1}@${scale.toFixed(3)}`)))) : (_jsx("div", { className: "grid h-full place-items-center text-sm text-muted-foreground", children: "Opening\u2026" })) })] })] }));
}
