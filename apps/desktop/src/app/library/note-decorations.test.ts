...
describe('note editor extensions mounted together (smoke test)', () => {
  ...
  const IMAGE_CONTEXT: ImageContext = {
    files: [{ name: 'cover.jpg', path: '/vault/Notes/cover.jpg' }],
    noteFolder: 'Notes',
    vaultDir: '/vault'
  }

  function mount(isResolved: (target: string) => boolean, imageContext: ImageContext = IMAGE_CONTEXT) {
  ...
  it('constructs without throwing and renders one checkbox per task line', () => {
    ...
    const boxes = view.dom.querySelectorAll<HTMLInputElement>('.cm-np-checkbox')
  ...
  it('toggles the checkbox character in the document when clicked', () => {
    ...
    const box = view.dom.querySelector<HTMLInputElement>('.cm-np-checkbox')
  ...
  it('renders a standard "![alt](path)" image resolved against the note folder', () => {
    ...
    const images = [...view.dom.querySelectorAll<HTMLImageElement>('.cm-np-image')]
