...
function applyMermaidTheme(mermaid: MermaidModule, dark: boolean): void {
...
function safeRequestMeasure(view: EditorView): void {
  try {
    view.requestMeasure()
  } catch {
...
class MermaidWidget extends WidgetType {
  ...
  toDOM(view: EditorView): HTMLElement {
    ...
    const showSource = (errored: boolean) => {
      wrap.innerHTML = ''
...
export function toggleTaskChar(current: string): string {
...
function findTableBlocks(doc: EditorView['state']['doc']): { from: number; to: number; header: string[]; rows: string[][] }[] {
  const blocks: { from: number; to: number; header: string[]; rows: string[][] }[] = []
...
export function findMermaidBlocks(doc: EditorView['state']['doc']): MermaidBlock[] {
...
function buildDecorations(
  ...
  for (const { from, to } of view.visibleRanges) {
    ...
    const insideProtected = (pos: number) => protectedRanges.some(range => pos >= range.from && pos < range.to)
    ...
    syntaxTree(view.state).iterate({
      enter: node => {
        ...
        if (type === 'TaskMarker') {
          ...
          decorations.push(
            Decoration.replace({ widget: new CheckboxWidget(isTaskChecked(marker), node.from + 1) }).range(
              node.from,
              node.to
            )
          )
        ...
        if (type === 'Image') {
          ...
          if (!selectionTouches(node.from, node.to)) {
            ...
            if (src) {
              ...
              decorations.push(
                Decoration.replace({ widget: new ImageWidget(src, altMatch?.[1] ?? '') }).range(node.from, node.to)
              )
            }
          }
    ...
    for (const match of text.matchAll(WIKILINK_RE)) {
      ...
      if (selectionTouches(start, end)) {
        ...
        decorations.push(
          Decoration.replace({ widget: new WikilinkWidget(label, target, isResolved(target), onOpen) }).range(
            start,
            end
          )
        )
