...
# Authoring Nemesis-Agent Skills (in-repo)
...
2. **Choose the right context load.** A model-invoked Nemesis skill pays for its description every turn. Keep descriptions focused on trigger classes and the skill's distinctive behavior. Put details in the body or linked references.
...
- Nemesis-specific recipes (tests via scripts/run_tests.sh, ui-tui paths, etc.)
